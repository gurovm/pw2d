# Spec 037 — AI model strategy: instrumentation, replay eval, and the `admin_model` migration

**Status:** T1 + T3b SHIPPED 2026-08-22 (code complete, tests green, AWAITING /deploy). T2 + T3 not started — T3 remains gated on T2.
...
The owner asked whether the AI spend justifies switching models — or providers (Claude, ChatGPT).
...
1. **The cost figure everyone quotes is wrong.** `ListProducts.php:39` tells the operator each retried
   ...
   real prompt shape, `evaluateProduct()` costs **~$0.0103** — roughly one third.
...
3. **Nothing is measured.** `GeminiService` discards `usageMetadata` entirely. No token count, no cost,
   no per-method attribution has ever been recorded. Every number in this spec — including the ones
   above — is an estimate derived from prompt length. **That is the actual defect.**
...
`evaluateProduct()` is the dominant call — one per imported product, on `admin_model`.
...
|---|---|---|---|---|---|
| **2.5 Pro** ← current | 1.25 | **10.00** | $0.0103 | **$3.59** | — |
| 3.1 Pro | 2.00 | 12.00 | $0.0132 | $4.62 | +29% |
| 3.5 Flash | 1.50 | 9.00 | $0.0099 | $3.47 | −3% |
| **3.7 Flash** ← recommended | 0.75 | 3.75 | $0.0044 | **$1.52** | **−58%** |
| 2.5 Flash | 0.30 | 2.50 | $0.0025 | $0.89 | −75% |
| 3.1 Flash-Lite | 0.25 | 1.50 | $0.0017 | $0.58 | −84% |
| 2.5 Flash-Lite | 0.10 | 0.40 | $0.0005 | $0.18 | −95% |

3.7 Flash's $0.75/$3.75 is promotional **through 2026-12-31**; it becomes $1.50/$7.50 on 2027-01-01 —
still cheaper than 2.5 Pro on output, which is where the spend actually sits (78% of per-product cost).

### 1.2 Across providers — the question the owner asked
...
- **Anthropic is the most expensive option at every tier for this workload.** Its cheapest model
  (Haiku 4.5, $1/$5) still costs **34% more than Gemini 3.7 Flash** and **2.8× gpt-5-mini**. Anthropic
...
- **The switching cost is real and one-directional.** See §5.

### 1.3 Batch API — considered and declined

All three providers offer **50% off** for asynchronous batch. The Bouncer is queue work, so it is a
natural fit: 2.5 Flash + batch would be **$0.45/category, −87%**.
...
### T1 — Usage instrumentation *(do this first; it is the only task with no prerequisite)*
...
- Attribute each call to its **caller method** — `AiService` passes a `string $purpose`
  (`evaluate_product`, `match_product`, `rescan_features`, `landing_page_content`, …) through to the
...
- Persist to a new `ai_usage` table. **`tenant_id` required, composite index leading with `tenant_id`**
  per the architect directive in `project_context.md` §11.

```
ai_usage
  id, tenant_id (string FK, nullable — console/central calls have no tenant),
...
- Pricing lives in `config/services.php` as `gemini.pricing.{model}.{input|output}` in $/M so the cost
  ...
  rather than throwing — **never let accounting break ingestion**.
...
§1's estimates can be replaced with measurements.

### T2 — Replay eval harness
...
1. Select N products with `status IS NULL`, `is_ignored` either value, that already have
   `ProductFeatureValue` rows — these are the golden set, already human-reviewed in practice.
2. Re-run `evaluateProduct()` against `--model` using each product's **stored offer `raw_title`,
   price, and category feature map** — the same inputs the original call received.
3. Diff against stored output and report:

| Metric | Why it matters |
|---|---|
| `is_ignored` agreement rate | The Bouncer's primary job. **The gate.** |
| Brand normalization exact-match rate | `RØDE` → `Rode`. Silent failure otherwise. |
...
**Ship gate for T3:** `is_ignored` agreement **≥ 95%**, brand exact-match **≥ 98%**, feature score MAD
**≤ 5 points** on a 0-100 scale. Below any of these, the candidate model is rejected and this spec's
T3 does not ship.

### T3 — `admin_model` migration to Gemini 3.7 Flash

Gated on T2 passing. The change is deliberately small:

- `.env` — `AGENT_ADMIN_MODEL=gemini-3.7-flash`. **The rollback is this one line**, which is why the
...
- **`thinking_level` handling in `GeminiService`.** Gemini 3 prefers `thinking_level`
  ...
  Translate at the transport boundary — `AiService` keeps expressing intent as a budget, and
...
**Blast radius — every `admin_model` caller re-validates, not just the Bouncer:** `evaluateProduct`,
...
**Watch for the upside:** `generateLandingPageContent` currently **times out on 2.5 Pro** — this is why
...
succeeding, re-test it explicitly and report** — it would retire a standing manual workaround. It must
...
- Switching the pipeline to Anthropic or OpenAI (§1.2 — declined, with reasons).
- Batch API (§1.3 — declined).
- Two-stage cheap-gate architecture (Flash-Lite triage → full scoring on survivors). It would cut
  ~76%, but of a $3.59 base. Not worth the complexity. **Reconsider only if `ai_usage` data from T1
  contradicts the cost model in §1.**
...
The measurement in §1 makes an option visible that was not obvious before.

`generateLandingPageContent` runs **~11 times per quarter** at roughly 3,000 in / 6,000 out. On Claude
Sonnet 5 that is **~$0.10 per page — about $1.10 per quarter for every landing page on the platform.**

The owner already writes this prose with Claude, by hand, because Gemini's admin model times out on the
call. Formalizing that — Gemini Flash for the high-volume Bouncer, Claude for the low-volume,
quality-dominated content generation — would automate an entire manual session per content cycle for
roughly a dollar.

**Deliberately not in this spec.** It requires a second transport class and a provider abstraction the
codebase does not have today (§5), and T3 may make it moot by fixing the timeout. **Decide after T3
...
1. `GeminiService` is Gemini-shaped throughout — endpoint URL, `x-goog-api-key`,
...
3. Prompts are Gemini-tuned — STAGE 1 gate, IGNORE RULE C, the grounding contract. All of it needs
...
4. JSON is parsed out of raw text. Anthropic and OpenAI both offer native structured outputs, which
   would be a genuine improvement — and a genuine rewrite.
...
Per `.claude/rules/standards.md` — Pest, `RefreshDatabase`, factories, no mocks except external APIs.

- `AiUsage` written on a successful call, with correct `purpose`, `tenant_id`, and computed cost.
- Usage-write failure (e.g. unknown model → null cost) does **not** fail the AI call.
- `usageMetadata` absent from the response → logs tokens as null, no exception.
- `thinkingBudget` → `thinking_level` translation for a Gemini 3 model; `thinkingBudget` preserved
  verbatim for a 2.5 model; **the two are never both present in one payload**.
- `pw2d:ai:eval-model` is read-only — assert `products` and `product_feature_values` are byte-identical
...
- T1 adds a migration → `/deploy` runs `migrate --force`. No extension change, no endpoint change, so
...
- T3 is `.env`-only on the server plus the `GeminiService` translation. **Do not change
  `AGENT_ADMIN_MODEL` on production until T2 has been run against production data** — the golden set
