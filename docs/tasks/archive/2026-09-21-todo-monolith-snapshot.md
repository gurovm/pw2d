# FROZEN SNAPSHOT — docs/tasks/todo.md as it stood on 2026-09-21 (1,539 lines)

> **Frozen, unmaintained, never read at boot.** This is the verbatim pre-migration task file: every item, open or
> closed, and every session narrative from 2026-03-22 to 2026-09-21. The live task files are `docs/tasks/todo.md`
> (active work) and `docs/tasks/backlog.md` (deferred). Lines in those files point back here by section heading when
> the detail matters. Items closed during the split, with the reason, are in
> `2026-09-21-closed-during-migration.md`. Do not edit this file; grep it.
