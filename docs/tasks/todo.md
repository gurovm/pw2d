...
- [ ] **Spec 024: Compare-page CWV initial-render weight cut (F31)** -- See `docs/specs/024-seo-compare-page-cwv-weight.md`. `@frontend`+`@builder`: render 6 cards server-side, hydrate next 6 on scroll (no CLS), preserve H2H/pinned/Load-more. `@tester` render-count tests. **ON HOLD** (owner) — measure 023 content effect in isolation first. *[Frontend, Perf]*

- [~] **Spec 025: Compare above-fold UX (CODE COMPLETE, awaiting commit + `/deploy`)** -- See `docs/specs/025-compare-abovefold-ux.md`. (1) reorder via new `partials/compare-content.blade.php` (intro/tabs/methodology moved BELOW grid; H1+hook stay above); (2) backdrop `bg-gray-900/40 backdrop-blur-[2px]` → `bg-gray-900/10` (no blur) so grid re-ranks visibly; (3) auto-open once per SESSION (`sessionStorage['app_customize_autoopen']`, 1500ms, respects `$autoOpen`, `customize_modal_autoopened` posthog event). **Tester caught + fixed a render-fatal bug:** `$categoryName` referenced in `x-init` attr before its `@php` assignment in the div body → `Undefined variable` on every compare page; fixed by moving `@php` above the `<div>`. 10 new tests, 199 passed / 0 failed. Frontend-only, no migration. **POST-DEPLOY: manual QA checklist** (auto-open ~1.5s, live re-rank no-blur, reload→teaser, new session→reopens, no auto-pop over product). *[Frontend, UX]*
