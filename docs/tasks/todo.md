...
- [ ] **F23: GSC single-call range request (growth-trigger)** -- `PullSeoMetrics` now issues 4 GSC API calls per tenant per night (one per date in the rolling window). Fine at current scale; at >200 tenants this approaches Google's per-project quota. Fix: use `searchanalytics.query` with a `startDate`/`endDate` range covering the full 4-day window, bucket results by date client-side. Cuts API calls 4x. Reference: `docs/performance/audit-2026-05-14-spec-016-performance.md` "High" section. *[Perf, Seo]*

- [ ] **F24: `SeoStatusCommand` SQL optimizations (growth-trigger)** -- Two related optimizations: (a) drop `whereIn` when no tenant filter is in play (trigger threshold: ~500 tenants — defeats range scan); (b) refactor `SUM(CASE WHEN metric_date >= ?)` to a two-subquery LEFT JOIN so MySQL can range-scan only the windowed slice instead of full `(tenant, source)` partition (trigger threshold: ~5M rows or if status becomes a hot path). Reference: `docs/performance/audit-2026-05-14-spec-016-performance.md` "Medium" section. *[Perf, Seo]*

- [ ] **F25: `pw2d:seo:pull` returns FAILURE when all sources upsert 0 rows (pre-existing from spec 014)** -- Inherited semantics: `$anySucceeded` only flips true if at least one tenant×source returned >0 rows with no errors. On a fresh install (GSC lag → 0 rows for the first few nights) or all-UNCONFIGURED state, the cron exits 1 every night. Not a regression — Spec 014 had the same behavior — but operationally surprising. Fix: distinguish "no data" from "errors" in the exit-code rule. Until then, `docs/seo/operations.md` should explicitly warn on-call about this. *[Seo, Ops]*
