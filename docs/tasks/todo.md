...
- [ ] **F14: `SeoMetric::$guarded = []` is vestigial** -- `app/Models/SeoMetric.php:48`. Production code uses `DB::table()->upsert()`, not Eloquent mass-assignment; the model is effectively read-only in widgets. Either replace with an explicit `$fillable` list, or add a comment explaining that `$guarded = []` is only exercised by `SeoMetricFactory` in tests. *[Review-Models]*
...
- [ ] **F19: Nightly cron should pull a GSC backfill window, not just yesterday** -- Google Search Console has a 2–3 day data lag: querying for "yesterday" usually returns an empty result set. Confirmed live on 2026-04-11 — yesterday returned 0 rows while 3 days ago returned data. Current `pw2d:seo:pull` cron (scheduled at 03:00 daily with default `--date=yesterday`) will persistently under-ingest GSC data. GA4 has no such lag so "yesterday" is fine there. Fix options: (a) change the cron's default date to 3-4 days ago (loses freshness on GA4); (b) iterate across a 4-day window [yesterday..3 days ago] in the command — the unique constraint makes re-inserts safe, so each night backfills any previously missed GSC days as data arrives; (c) split the GSC and GA4 pulls so each uses its own optimal lag. Option (b) is cleanest and ~10 LOC change in `PullSeoMetricsCommand::handle()`. *[Scheduler, Seo]*
