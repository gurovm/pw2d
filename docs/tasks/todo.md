...
- [x] **Spec 023: frontend + tests + review (CODE COMPLETE, awaiting `/deploy`)** -- Frontend: `product-compare.blade.php` intro priority-chain (preset → category) + `partials/compare-faqs.blade.php` preset-first case-insensitive deduped FAQs. Tester: 36 new tests (`tests/Feature/Ai/GeneratePresetContentTest.php`, `tests/Feature/Commands/GeneratePresetContentCommandTest.php`, `tests/Feature/Compare/PresetContentDepthTest.php`) + `PresetFactory::seoContent()` state — all green, 0 bugs. Reviewer: SHIP / 0 blockers (`docs/reviews/023-preset-content-review.md`). Fixes applied: S1 (dropped redundant active-preset query via optional `?Preset` param on SeoSchema), N2 (aligned FAQ dedup case-insensitive across schema+Blade), N3 (`strict_types` on Preset). Final: 232 `Seo|Ai|Compare` tests pass. **POST-DEPLOY:** deploy runs the migration; then `pw2d:generate-preset-content pw2d --dry-run` → review copy for streamer/remote-worker/minimalist → populate for real. *[Seo, AI, Frontend, Models]*

- [ ] **Spec 024: Compare-page CWV initial-render weight cut (F31)** -- See `docs/specs/024-seo-compare-page-cwv-weight.md`. `@frontend`+`@builder`: render 6 cards server-side, hydrate next 6 on scroll (no CLS), preserve H2H/pinned/Load-more. `@tester` render-count tests. Fast-follow AFTER 023 lands. *[Frontend, Perf]*
