<div>
    <div class="bg-gradient-to-br from-gray-50 to-white min-h-screen">
        ...
        <livewire:comparison-header 
            :features="$features" 
            :weights="$weights" 
            :priceWeight="$priceWeight" 
            :amazonRatingWeight="$amazonRatingWeight"
            :categoryId="$category->id"
            :autoOpen="!$selectedProductSlug"
        />

        <!-- Category Header (Merged with Buying Guide) -->
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                <div class="cat-header-grid">
                    ...
                    <div class="cat-header-left">
                        @if($category->image)
                            <img src="{{ asset('storage/' . $category->image) }}" alt="{{ $category->name }}" class="w-full h-full object-cover rounded-2xl">
                        @else
                            <div class="w-full h-full bg-gradient-to-br from-indigo-100 to-blue-50 rounded-2xl flex items-center justify-center">
                                <svg class="w-16 h-16 text-indigo-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path></svg>
                            </div>
                        @endif
                    </div>

                    <!-- Right: Title + Buying Guide -->
                    <div class="cat-header-right">
                        <h1 class="text-2xl sm:text-3xl font-black text-gray-900 tracking-tight mb-3">{{ $category->name }}</h1>
                        ...
                        @if($category->buying_guide && is_array($category->buying_guide))
                            @php
                                $sections = [
                                    'how_to_decide' => [
                                        ...
                                        'content' => $category->buying_guide['how_to_decide'] ?? ''
                                    ],
                                    'the_pitfalls' => [
                                        'title' => 'The Pitfalls',
                                        'icon' => '<svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>',
                                        'activeColor' => 'text-rose-500',
                                        'content' => $category->buying_guide['the_pitfalls'] ?? ''
                                    ],
                                    'key_jargon' => [
                                        'title' => 'Key Jargon',
                                        'icon' => '<svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg>',
                                        'activeColor' => 'text-indigo-500',
                                        'content' => $category->buying_guide['key_jargon'] ?? ''
                                    ]
                                ];
                            ...
                            <div x-data="{ expanded: false, activeTab: '{{ $defaultTab }}' }">
                                <div class="bg-gradient-to-br from-slate-50 via-white to-indigo-50/30 rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                                    <!-- Tabs -->
                                    <div class="flex items-center gap-2 px-4 pt-3 pb-1.5 border-b border-gray-100/80 bg-white/60">
                                        <div class="flex space-x-1 overflow-x-auto scrollbar-hide w-full pb-1">
                                            @foreach($sections as $key => $data)
                                                @if(!empty($data['content']))
                                                    <button @click="activeTab = '{{ $key }}'; expanded = true"
                                                            ...
                                                            :class="activeTab === '{{ $key }}' ? 'text-gray-900 font-bold bg-gray-50/50' : 'text-gray-500 font-semibold hover:text-gray-800'">
                                                        <span :class="activeTab === '{{ $key }}' ? '{{ $data['activeColor'] }}' : 'text-gray-400'">
                                                            {!! $data['icon'] !!}
                                                        </span>
                                                        {{ $data['title'] }}
                                                    </button>
                                    ...
                                    <div class="relative overflow-hidden"
                                         :style="expanded ? 'max-height: 2000px' : 'max-height: 5.5rem'"
                                    ...
                                    <!-- Footer with gradient fade + toggle -->
                                    <div class="relative">
                                        <div x-show="!expanded" class="absolute -top-10 left-0 right-0 h-10 bg-gradient-to-t from-white via-white/80 to-transparent pointer-events-none transition-opacity duration-300"></div>
                                        <div class="px-4 pb-3 pt-1.5 border-t border-gray-100/50 flex justify-between items-center bg-white/50 backdrop-blur-sm">
                                            <button @click="expanded = !expanded" 
                                                ...
                                                <span x-text="expanded ? 'Show less' : 'Read full guide'"></span>
                                                <svg class="w-3.5 h-3.5 transition-transform duration-300" :class="expanded ? 'rotate-180' : 'group-hover:translate-y-0.5'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"></path></svg>
                                            </button>
                                        </div>
                                    </div>
                                </div>
        ...
        <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 py-4">
            ...
            <div class="mb-6 flex flex-wrap items-center gap-3">
                ...
                <!-- Custom Searchable Brand Filter -->
                <div x-data="{ 
                        open: false, 
                        search: '',
                    ...
                    <button @click="open = !open" 
                        ...
                        <svg class="h-4 w-4 ml-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                    </button>
                    ...
                    <div x-show="open" 
                        ...
                        <!-- Brand List -->
                        <div class="max-h-60 overflow-y-auto overscroll-contain pb-1 custom-scrollbar">
                            <button @click="selectedBrandId = ''; open = false; search = ''" 
                                    ...
                                    :class="!selectedBrandId ? 'bg-blue-50/50 text-blue-700 font-semibold' : 'text-gray-700'">
                                <span>All Brands</span>
                                <span class="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full font-medium">{{ $this->availableBrands->sum('products_count') }}</span>
                            </button>
                            ...
                            <template x-for="brand in filteredBrands" :key="brand.id">
                                <button @click="selectedBrandId = brand.id; open = false;" 
                                        ...
                                        :class="selectedBrandId == brand.id ? 'bg-blue-50/50 text-blue-700 font-semibold' : 'text-gray-700'">
                                    <span x-text="brand.name"></span>
                                    <span class="text-xs px-2 py-0.5 rounded-full font-medium"
                                          :class="selectedBrandId == brand.id ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'"
                                          x-text="brand.count"></span>
                                </button>
                            </template>
                            
                            <!-- Empty Search Result -->
                            <div x-show="filteredBrands.length === 0" class="px-4 py-4 text-center text-sm text-gray-500">
                                No brands found
                            </div>
                        </div>
                    </div>
            ...
            @if($products->count() > 0 && $features->count() > 0)
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-1.5 md:gap-5" x-data x-init="import('https://cdn.jsdelivr.net/npm/@formkit/auto-animate').then(module => module.default($el))">
                    @foreach($products->take($displayLimit) as $product)
                        <div wire:key="product-{{ $product->id }}" class="product-card bg-white rounded-2xl shadow-sm hover:shadow-[0_12px_40px_rgba(255,153,0,0.2)] transition-all duration-300 overflow-hidden flex flex-col h-full border hover:border-amber-400 group relative {{ $loop->first ? 'border-2 border-amber-400 shadow-[0_4px_20px_rgba(245,158,11,0.15)]' : 'border-gray-100' }}">
                            ...
                            <!-- Amazon CTA (full-width bar at bottom) -->
                ...
                @if($products->count() > $displayLimit)
    ...
    @if($this->selectedProduct)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 pt-20 sm:p-6 sm:pt-20"
            ...
            <div class="fixed inset-0 bg-gray-900/60 backdrop-blur-md transition-opacity" 
                 @click="window.history.pushState({}, '', '/compare/{{ $category->slug }}'); $wire.closeProduct()"></div>
            
            <!-- Modal Window (Split View Container) -->
            <div class="relative w-full max-w-5xl bg-white rounded-2xl shadow-2xl overflow-y-auto md:overflow-hidden flex flex-col md:flex-row max-h-[85vh] z-10"
                 x-show="show" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0 translate-y-8 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="transition ease-in duration-200 transform" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
                 
                <!-- Floating Close Button -->
                <button type="button" @click="window.history.pushState({}, '', '/compare/{{ $category->slug }}'); $wire.closeProduct()" class="absolute top-4 right-4 z-50 p-2.5 bg-white/80 hover:bg-white backdrop-blur-md rounded-full text-gray-500 hover:text-gray-900 transition-all focus:outline-none focus:ring-2 focus:ring-gray-300 shadow-sm border border-gray-100">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>

                <!-- Left Column: Image & Primary CTA (40% width, sticky) -->
                <div class="w-full md:w-[40%] bg-gray-50 flex flex-col items-center justify-between p-4 md:p-8 relative h-auto md:sticky md:top-0 border-r border-gray-100 shrink-0">
                    <div class="flex-1 flex items-center justify-center w-full min-h-[200px] md:min-h-[250px] mb-4 md:mb-8">
                        @if($this->selectedProduct->image_url)
                            <img src="{{ $this->selectedProduct->image_url }}" alt="{{ $this->selectedProduct->name }}" class="w-full max-h-[300px] object-contain mix-blend-multiply drop-shadow-xl hover:scale-105 transition-transform duration-700 ease-out">
                        @else
                            <svg class="w-24 h-24 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                        @endif
                    </div>
                    
                    <!-- Moved Sticky Bottom CTA here -->
                    @if($this->selectedProduct->affiliate_url)
                        <div class="w-full flex flex-col items-center gap-3 md:gap-4 bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-gray-100">
                            <div class="flex flex-col items-center text-center">
                                ...
                                @if($this->selectedProduct->price_tier)
                                    <div class="text-lg md:text-2xl font-bold tracking-tight text-gray-900">
                                        @if($this->selectedProduct->price_tier == 1) <span class="text-emerald-500 mr-0.5 text-base md:text-lg">$</span> Budget
                                        @elseif($this->selectedProduct->price_tier == 2) <span class="text-emerald-500 mr-0.5 text-base md:text-lg">$$</span> Mid-Range
                                        @else <span class="text-emerald-500 mr-0.5 text-base md:text-lg">$$$</span> Premium
                                        @endif
                                    </div>
                                @endif
                            </div>
                            <a href="{{ $this->selectedProduct->affiliate_url }}" target="_blank" rel="noopener noreferrer" 
                ...
                <!-- Right Column: Details (60% width, Scrollable) -->
                <div class="w-full md:w-[60%] md:overflow-y-auto p-5 sm:p-6 md:p-10 custom-scrollbar flex flex-col relative bg-white">
                    <style>
                    ...
                    <div class="flex items-center space-x-3 mb-3 md:mb-4">
                        <span class="px-3 py-1 bg-gray-100 text-gray-800 text-xs font-bold uppercase tracking-widest rounded-full">{{ $this->selectedProduct->brand->name }}</span>
                        @if($this->selectedProduct->amazon_rating)
                            <div class="flex items-center text-sm font-semibold text-gray-700 shrink-0">
                                ...
                                <span class="text-gray-400 font-normal ml-1">({{ number_format($this->selectedProduct->amazon_reviews_count) }})</span>
                            </div>
                        @endif
                    </div>
                    
                    <h2 class="text-2xl md:text-4xl font-extrabold text-gray-900 tracking-tight leading-tight mb-4 md:mb-6">{{ $this->selectedProduct->name }}</h2>
                    
                    <!-- Modern Gradient Card: AI Verdict -->
                    @if($this->selectedProduct->ai_summary)
                    <div class="bg-indigo-50 border-l-4 border-indigo-500 p-4 rounded-r-lg mb-6 shadow-sm">
                        <div class="flex items-center gap-2.5 mb-2">
                            ...
                            <h4 class="text-xs font-bold text-indigo-900 uppercase tracking-widest">The Verdict</h4>
                        </div>
                        <p class="text-indigo-900 text-sm leading-relaxed font-medium relative z-10 whitespace-pre-line">{{ $this->selectedProduct->ai_summary }}</p>
                    </div>
                    @endif
                    
                    <!-- Circular Match Score -->
                    @php
                        $matchScore = $matchScores[$this->selectedProduct->id] ?? 0;
                        $scoreColor = $matchScore >= 85 ? 'text-emerald-500' : ($matchScore >= 70 ? 'text-blue-500' : ($matchScore >= 50 ? 'text-yellow-500' : 'text-rose-500'));
                    @endphp
                    <div class="mb-8 md:mb-10 flex items-center gap-4 md:gap-6 p-4 md:p-6 bg-gray-50/50 rounded-2xl border border-gray-100">
                        <div class="relative w-16 h-16 md:w-24 md:h-24 flex items-center justify-center shrink-0">
                            <svg class="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                                <path class="text-gray-200" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round"></path>
                                <path class="{{ $scoreColor }} transition-all duration-[1500ms] ease-out" 
                                      x-data="{ score: 0 }"
                                      x-intersect="setTimeout(() => { score = {{ $matchScore }} }, 100)"
                                      :stroke-dasharray="score + ', 100'" 
                                      d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round"></path>
                            </svg>
                            <div class="absolute inset-0 flex flex-col items-center justify-center">
                                <span class="text-2xl font-bold text-gray-900 tracking-tighter">{{ number_format($matchScore, 0) }}<span class="text-sm font-semibold">%</span></span>
                            </div>
                        </div>
                        <div class="flex-1">
                            <h4 class="text-sm font-bold text-gray-900 mb-1">Personalized Match</h4>
                            <p class="text-sm text-gray-500 leading-relaxed">Based on your precise slider configurations, this product has been objectively rated at <strong class="text-gray-900">{{ number_format($matchScore, 1) }}%</strong> compatibility for your exact needs.</p>
                        </div>
                    </div>
                    
                    <!-- Premium Feature Specs -->
                    <div class="mb-12">
                        <h4 class="text-sm font-bold text-gray-900 border-b border-gray-100 pb-3 mb-6">Technical Specifications</h4>
                        <div class="space-y-5">
                            @foreach($features as $feature)
                                @php
                                    $normalizedScore = $featureScores[$this->selectedProduct->id][$feature->id] ?? null;
    ...
    <!-- PostHog Tracking for AI Concierge -->
    <script>
