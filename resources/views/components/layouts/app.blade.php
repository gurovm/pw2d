...
<head>
        ...
        <meta property="og:description" id="og-description" content="{{ $defaultDescription }}"
                data-default="{{ $defaultDescription }}">
        ...
        <meta name="twitter:title" id="twitter-title" content="{{ $defaultTitle }}"
                data-default="{{ $defaultTitle }}">
        <meta name="twitter:description" id="twitter-description" content="{{ $defaultDescription }}"
                data-default="{{ $defaultDescription }}">
                ...
                <script>
                        ! function(t, e) {
                                ...
                                e.__SV || (window.posthog = e, e._i = [], e.init = function(i, s, a) {
                                        ...
                                        }(p = t.createElement("script")).type = "text/javascript", p.async = !0, p.src =
                                                s.api_host + "/static/array.js", (r = t.getElementsByTagName("script")[
                                                        0]).parentNode.insertBefore(p, r);
                                        var u = e;
                                        for (void 0 !== a ? u = e[a] = [] : a = "posthog", u.people = u.people || [], u
                                                .toString = function(t) {
                                                        var e = "posthog";
                                                        return "posthog" !== a && (e += "." + a), t || (e += " (stub)"),
                                                                e
...
<body class="antialiased bg-white text-deep-blue" style="overflow-x:hidden; max-width:100vw;">
                ...
                <div class="border-b border-gray-700 bg-gray-800">
                        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 text-center">
                                <p class="text-amber-400 text-xs">
                                        ...
                                        <span class="text-gray-400 font-normal"> {{ tenant('brand_name') ?? 'Our site' }}
                                                is reader-supported. When you buy through links on our site, we may earn
                                                an affiliate commission at no extra cost to you.</span>
                                </p>
                        </div>
                </div>
                ...
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
                        <div class="flex flex-col sm:flex-row items-center justify-between gap-6">
                                ...
                                <div class="flex flex-col items-center sm:items-start">
                                        <span
                                                class="text-white font-bold text-lg tracking-tight">{{ tenant('brand_name') ?? 'Pw2D' }}</span>
                                        <span
                                                class="text-gray-400 text-xs mt-0.5">{{ tenant('name') ?? 'Power to Decide' }}</span>
                                </div>

                                <!-- Nav Links -->
                                <nav class="flex flex-wrap justify-center gap-x-6 gap-y-2">
                                        <a href="{{ route('about') }}"
                                                class="text-gray-400 hover:text-white text-sm transition-colors duration-150">About</a>
                                        <a href="{{ route('contact') }}"
                                                class="text-gray-400 hover:text-white text-sm transition-colors duration-150">Contact</a>
                                        <a href="{{ route('privacy-policy') }}"
                                                class="text-gray-400 hover:text-white text-sm transition-colors duration-150">Privacy
                                                Policy</a>
                                        <a href="{{ route('terms-of-service') }}"
                                                class="text-gray-400 hover:text-white text-sm transition-colors duration-150">Terms
                                                of Service</a>
                                </nav>
                        </div>
                        ...
                        <div class="border-t border-gray-800 mt-8 pt-6 text-center">
                                <p class="text-gray-400 text-xs">&copy; {{ date('Y') }}
                                        {{ tenant('brand_name') ?? 'Pw2D' }} &mdash;
                                        {{ tenant('name') ?? 'Power to Decide' }}. All rights reserved.</p>
                        </div>
                </div>
        </footer>
        ...
        <script>
                document.addEventListener('livewire:initialized', () => {
                        ...
                        Livewire.on('meta:product-opened', ({
                                title,
                                description,
                                image,
                                url
                        }) => {
                                document.getElementById('og-type')?.setAttribute('content',
                                        'product');
                        ...
                        Livewire.on('meta:product-closed', () => {
                                ...
                                ['og-title', 'og-description', 'og-url', 'og-image',
                                        ...
                                        if (el) el.setAttribute('content', el
                                                .dataset.default ?? '');
                                });
                                const pageTitle = document.getElementById('og-title')?.dataset
                                        .default ?? '';
